...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    plt.axvline(x=slope, color='blue', linestyle='--', linewidth=2, label='Observed Slope')
    plt.axvline(x=intercept, color='orange', linestyle='--', linewidth=2, label='Observed Intercept')
    plt.legend()
...
def hypothesis_test():
    ...
    if test_type == ">":
        ...
        deviation = np.abs(observed_stat - hypothesized_value)
        extreme_high = simulated_stats >= (hypothesized_value + deviation)
        extreme_low = simulated_stats <= (hypothesized_value - deviation)
        p_value = (np.sum(extreme_high) + np.sum(extreme_low)) / S




    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    fun_message = "Wow, that's a rare event!" if p_value <= 0.0001 else "This is a common occurrence."
    ...
    plt.figure()
    plt.hist(simulated_stats, bins=30, color='grey', alpha=0.5)
    plt.axvline(x=observed_stat, color='red', linestyle='dashed', linewidth=2, label='Observed Slope')
    plt.axvline(x=hypothesized_value, color='blue', linestyle='dashed', linewidth=2, label='Hypothesized Slope')
...
def confidence_interval():
    ...
    std_estimate = np.std(estimates, ddof=1) / np.sqrt(len(estimates))
    ...
    plt.axvline(x=true_param, color='green', linestyle='--', label='True Slope')
